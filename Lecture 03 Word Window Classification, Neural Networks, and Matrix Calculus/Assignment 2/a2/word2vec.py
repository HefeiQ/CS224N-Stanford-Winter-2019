...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    z = np.dot(outsideVectors, centerWordVec) # score vector z
    y_hat = softmax(z)
    delta = y_hat.copy()
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    ### YOUR CODE HERE
    current_center_id = word2Ind[currentCenterWord]
    vector_center = centerWordVectors[current_center_id]

    for outside_word in outsideWords:
        outsideWordIdx = word2Ind[outside_word]
        each_loss, each_grad_center, each_grad_out =\
        word2vecLossAndGradient(vector_center, outsideWordIdx, outsideVectors, dataset)
        loss += each_loss
        # update
        gradCenterVecs[current_center_id] += each_grad_center
